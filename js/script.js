...
const displayAlbums = async () => {
  ...
  let responce = await fetch(`https://drive.google.com/drive/folders/191IB15HVPZG3ciUO9I8-wWoVJMCSRTif?usp=sharing`);
  ...
  for (const folder of folders) {
    ...
    let responce = await fetch(
      `https://drive.google.com/drive/folders/191IB15HVPZG3ciUO9I8-wWoVJMCSRTif?usp=sharing${folder}/info.json`
    );
    let folderInfo = await responce.json();
    // console.log(folderInfo);
    card.innerHTML = `
        <div><img src="https://drive.google.com/drive/folders/191IB15HVPZG3ciUO9I8-wWoVJMCSRTif?usp=sharing${folder}/cover.jpg" alt="cover"></div>
        <div class="details">
...
const loadMusic = async (folder) => {
  ...
  let responce = await fetch(`https://drive.google.com/drive/folders/191IB15HVPZG3ciUO9I8-wWoVJMCSRTif?usp=sharing${folder}/`);
  let text = await responce.text();
  let doc = document.createElement(`div`);
  doc.innerHTML = text;
  let anchors = Array.from(doc.querySelectorAll(`a`));
...
const playMusic = (file, audio) => {
  currAudio.pause();
  currAudio = new Audio(`https://drive.google.com/drive/folders/191IB15HVPZG3ciUO9I8-wWoVJMCSRTif?usp=sharing${file}/${audio}`);
  currAudio.play();
